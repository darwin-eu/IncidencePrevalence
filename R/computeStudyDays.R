...
#' @param fullPeriodsRequired whether full periods are required, if a period is given
...
computeStudyDays <- function(startDate,
  ...
  if (timeInterval == "days") {
    studyDays <- dplyr::tibble(start_time = seq.Date(from = startDate,
                                                     to = endDate,
                                                     by = "days")) %>%
    ...
    if (endDate >= startDay){
      studyDays<-dplyr::tibble(start_time = seq.Date(from = startDay,
                                                     to = endDate,
                                                     by = timeInterval)) %>%
        ...
        dplyr::rename("time" = timeInterval) %>%
    ...
    studyDays <- dplyr::tibble(dates = seq.Date(from = startDate,
                                                to = endDate,
                                                by = "days")) %>%
    ...
    if (fullPeriodsRequired){
      studyDays <- studyDays %>%
        dplyr::filter(.data$start_time == lubridate::floor_date(.data$start_time, unit = timeInterval) + week_correction) %>%
        dplyr::filter(.data$end_time == lubridate::floor_date(
          .data$end_time,
          unit = timeInterval
        ) + week_correction + switch(timeInterval,
                                     "weeks" = lubridate::days(6),
                                     "months" = months(1) - lubridate::days(1),
                                     "quarters" = months(3) - lubridate::days(1),
                                     "years" = lubridate::years(1) - lubridate::days(1)
