calculate_pop_incidence <- function(db,
  ...
  for (i in 1:(n.time + 1)) {
    ...
    for(z in 1:length(outcomes.patients)){
      id <-as.numeric(outcomes.patients[z])
      seq.max <- outcomes.all %>%filter(person_id==id)%>%ungroup()%>%select(seq)%>%pull()%>%max()
      if(seq.max==1){
        outcomes.z[[paste0(z)]]  <- outcomes.all %>%filter(person_id==id)%>%slice(rep(1:n(), each =seq.max+1))
      }else{
        outcomes.z[[paste0(z)]]  <- rbind(outcomes.all %>%filter(person_id==id)%>%slice(1),
                                          outcomes.all %>%filter(person_id==id)%>%slice(rep(seq.max:n(), each =2)))
      }
    }
    ...
    for(y in 1:length(outcomes.patients)){
     ...
     patient.rows <- outcomes.z %>% filter(person_id==id)%>%arrange(seq.z)
     outcomes.dates <- patient.rows %>%distinct(outcome_date)%>% pull()
     patient.rows$t_end_date[1]<-patient.rows$outcome_date[1]
        for(q in 2:NROW(patient.rows)){
          patient.rows$t_start_date[q]<-patient.rows$t_end_date[q-1]+days(prior_event_lookback)
          if(q < NROW(patient.rows)){
          patient.rows$t_end_date[q]<-patient.rows$outcome_date[q]
          } else {
            patient.rows$outcome_date[q]<-NA
          }
        }
