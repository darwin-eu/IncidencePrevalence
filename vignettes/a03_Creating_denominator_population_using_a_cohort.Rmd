...
## Introduction
The `collectDenominator()` function is designed to identify the individuals and the time their satisfy a set different set of criteria (age, sex, prior amount of prior history observed) in a particular time period.
...
Subset cohorts must be inserted in the server before running the `collectDenominator()` function. They can be generated based on different strategies, such as by bespoke code or by using ATLAS. 
...
When setting the `collectDenominator()` arguments, we'll need to specify the schema and table where our subset cohort is recorded (strata_schema and strataTable). In case we have multiple subsets, we'll also need to specify the identification of the particular strata we're interested in (strataId). 
...
## Using collectDenominator() against a population subgroup
To demonstrate its use, let´s load the IncidencePrevalence package (along with a couple of packages to help for subsequent plots) and generate 10 example patients using the `mockIncidencePrevalenceRef()` function.
...
Let's suppose we want to subset our population based on a non-time varying characteristic such as ethnicity, which corresponds to strataId "1" in our simulated strata table.
...
Now we want to subset our population based on a time varying characteristic such a particular condition (strataId "2" in our simulated strata table).
...
Depending in which condition we're interested in, people might experience the same condition multiple times. Let's use strataId "3" to illustrate this example. 
