...
## Introduction
The `generateDenominatorCohortSet()` function is designed to identify the individuals and the time their satisfy a set different set of criteria (age, sex, prior amount of prior history observed) in a particular time period.
...
Subset cohorts must be inserted in the server before running the `generateDenominatorCohortSet()` function. They can be generated based on different strategies, such as by bespoke code or by using ATLAS. 
