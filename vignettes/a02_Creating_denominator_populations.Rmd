...
# Introduction
Calculating incidence or prevalence requires first identifying an appropriate denominator population. To find such a denominator population (or multiple denominator populations) we can use the `collect_denominator_pops()` function. This function will identify the time that people satisfy a set of criteria related to the study period and individuals´ age, sex, and amount of prior observed history. 

The `collect_denominator_pops()` function is designed to be used with data mapped to the OMOP CDM format. Unless otherwise specified (see the next vignette "Creating denominator population using a cohort"), it will use information from the person and observation_period tables of the OMOP CDM to identify denominator populations. To note, identifying the denominator population does not require any information on specific outcome occurrences. 

When using `collect_denominator_pops()`, individuals will enter a denominator population on the respective date of the latest of the following: 
...
# Using collect_denominator_pops()
`collect_denominator_pops()` is the function we use to identify a set of denominator populations. To demonstrate its use, let´s load the IncidencePrevalence package (along with a couple of packages to help for subsequent plots) and generate 500 example patients using the `generate_mock_incidence_prevalence_db()` function.
...
db<-generate_mock_incidence_prevalence_db(sample_size=500)
```
...
dpop %>%
  filter(person_id %in% c("1", "2", "3", "4", "5"))  %>% 
  pivot_longer(cols = c("cohort_start_date",
                        "cohort_end_date")) %>%
  ggplot()+
  geom_point(aes(x=value,  y=person_id))+
  geom_line(aes(x=value,  y=person_id))+
  theme_minimal()+
  xlab("Year")

```

We can also plot a histogram of start and end dates
```{r, message=FALSE, warning=FALSE}
dpop %>% 
  ggplot() + 
  theme_minimal()+
...
dpop <- collect_denominator_pops(db = db,
                                 ...
                                 study_end_date= as.Date("2010-01-01"),
                                 study_age_stratas = NULL,
                                 study_sex_stratas = "Both",
                                 study_days_prior_history = 0)
dpop %>%  glimpse()
...
Now we can see the person "3" and "5" haven´t been included as they don´t have any observation time during the study period. Indeed, we´re now including 328 of the original 500 simulated patients.
...
Now we only include patient "4" of the original first five and we´re now including 178 of the original 500 simulated patients.
...
### Specified study period, prior history requirement, and age and sex criteria
In addition to all the above we could also add some requirements around age and sex. One thing to note is when the upper limit is 65 below, that means we will include time from a person up to and including the day before their 66th birthday.
...
More than one age, sex and prior history requirements can be specified at the same time. First, we can take a look at having two age strata. We can see below that those individuals who have their 41st birthday during the study period will go from the first cohort (age_strata: 0;40) to the second (age_strata: 41;100) on this day.
```{r, message=FALSE, warning=FALSE}
dpop <- collect_denominator_pops(db = db,
                                 cdm_database_schema = NULL,
                                 study_start_date= as.Date("2008-01-01"),
                                 study_end_date= as.Date("2010-01-01"),
                                 study_age_stratas = list(c(0,40),
                                                          c(41,100)),
                                 study_sex_stratas = "Both",
                                 study_days_prior_history = 0)
...
dpop %>% group_by(cohort_definition_id, age_strata) %>% tally()

dpop %>%
  filter(person_id %in% c("1","3","57", "353", "393", "496"))  %>% 
  pivot_longer(cols = c("cohort_start_date",
                        "cohort_end_date")) %>%
  ggplot(aes(x=person_id,  y=value, colour=cohort_definition_id))+
...
dpop %>% group_by(cohort_definition_id, age_strata, sex_strata) %>% tally()
...
dpop %>%
  ...
  pivot_longer(cols = c("cohort_start_date",
                        "cohort_end_date")) %>%
  ggplot(aes(x=person_id,  y=value, colour=cohort_definition_id))+
  facet_grid(sex_strata ~ ., space = "free_y") +
  geom_point(position=position_dodge(width=0.5))+
  geom_line(position=position_dodge(width=0.5))+
  theme_bw()+
  theme(legend.position = "top")+
...
dpop %>% 
  group_by(cohort_definition_id, age_strata, sex_strata, required_days_prior_history) %>% 
  tally()
...
dpop %>%
  ...
  pivot_longer(cols = c("cohort_start_date",
                        "cohort_end_date")) %>%
  ggplot(aes(x=person_id,  y=value, colour=cohort_definition_id))+
  facet_grid(sex_strata+required_days_prior_history ~ ., space = "free_y") +
  geom_point(position=position_dodge(width=0.5))+
  geom_line(position=position_dodge(width=0.5))+
  theme_bw()+
  theme(legend.position = "top")+
