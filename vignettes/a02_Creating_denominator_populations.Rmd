...
# Introduction
Calculating incidence or prevalence requires first identifying an appropriate denominator population. To find such a denominator population (or multiple denominator populations) we can use the `collectDenominatorPops()` function. This function will identify the time that people satisfy a set of criteria related to the study period and individuals´ age, sex, and amount of prior observed history. 

The `collectDenominatorPops()` function is designed to be used with data mapped to the OMOP CDM format. Unless otherwise specified (see the next vignette "Creating denominator population using a cohort"), it will use information from the person and observation_period tables of the OMOP CDM to identify denominator populations. To note, identifying the denominator population does not require any information on specific outcome occurrences. 

When using `collectDenominatorPops()`, individuals will enter a denominator population on the respective date of the latest of the following: 
...
# Using collectDenominatorPops()
`collectDenominatorPops()` is the function we use to identify a set of denominator populations. To demonstrate its use, let´s load the IncidencePrevalence package (along with a couple of packages to help for subsequent plots) and generate 500 example patients using the `generate_mock_incidence_prevalence_db()` function.
...
cdm <- generate_mock_incidence_prevalence_db(sample_size = 500)
```
...
dpop %>% 
  group_by(cohort_definition_id, age_strata, sex_strata, days_prior_history) %>% 
  tally()
...
dpop %>%
  ...
  pivot_longer(cols = c("cohort_start_date",
                        "cohort_end_date")) %>%
  ggplot(aes(x=subject_id,  y=value, colour=cohort_definition_id))+
  facet_grid(sex_strata+days_prior_history ~ ., space = "free_y") +
  geom_point(position=position_dodge(width=0.5))+
  geom_line(position=position_dodge(width=0.5))+
  theme_bw()+
  theme(legend.position = "top")+
...
The `collectDenominatorPops()` function can also be run for a subset of the population with a particular characteristic recorded in our database, such as sociodemographic groups (ethnicity, socioeconomic status...), conditions or users of a particular medication, among others. This feature is further explained in the next vignette (see the vignette "Creating denominator population using a cohort"). 
