...
`collectDenominatorPops()` is the function we use to identify a set of denominator populations. To demonstrate its use, let´s load the IncidencePrevalence package (along with a couple of packages to help for subsequent plots) and generate 500 example patients using the `mockIncidencePrevalenceRef()` function.
...
dpop %>%
  group_by(cohort_definition_id, age_strata) %>%
  tally()
...
dpop %>%
  group_by(cohort_definition_id, age_strata, sex_strata) %>%
  tally()
