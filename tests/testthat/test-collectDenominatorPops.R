...
test_that("mock db: check output format", {
  ...
  # variable names
  expect_true(length(names(dpop$denominator_population %>%
                             dplyr::collect())) == 4)
  expect_true(all(c(
  ...
  # check verbose
  expect_message(collectDenominatorPops(
    cdm = cdm,
    verbose = TRUE
  ))
...
test_that("mock db: checks on working example", {
  ...
  dpops <- collectDenominatorPops(cdm,
    startDate = NULL,
    endDate = NULL,
  ...
  expect_message(dpops <- collectDenominatorPops(cdm,
    startDate = NULL,
    endDate = NULL,
...
test_that("mock db: mock example 10000", {
  ...
  expect_true(nrow(dpops$denominator_population %>%
    dplyr::collect()) > 0)
  ...
  expect_true(nrow(dpops$denominator_population %>%
    dplyr::collect()) > 0)
  expect_true(min(dpops$denominator_populations %>%
    dplyr::collect() %>%
    dplyr::pull(cohort_start_date)) >=
    as.Date("2011-01-01"))
  expect_true(max(dpops$denominator_populations %>%
    dplyr::collect() %>%
...
test_that("mock db: check example with restriction on sex", {
  ...
  dpop1 <- collectDenominatorPops(
    cdm = cdm,
    sexStrata = "Male"
  )
  dpop2 <- collectDenominatorPops(
    cdm = cdm,
    sexStrata = "Both"
  )
  dpop3 <- collectDenominatorPops(
    cdm = cdm,
    sexStrata = "Female"
  )
...
test_that("mock db: check expected errors", {
  ...
  # not a cdm reference
  testthat::expect_error(collectDenominatorPops(
    cdm = "a"
  ))
