test_that("check for incidence", {
  inc <- tibble::tibble(
    ...
    person_days = 100,
    person_years = 1,
    ir_100000_pys = c(1, 25, 50),
    ir_100000_pys_low = c(0.1, 20, 50),
    ir_100000_pys_high = c(10, 30, 50)
  )
  ...
  expect_true(!is.na(inc$ir_100000_pys[3]))
  expect_true(!is.na(inc$ir_100000_pys_low[3]))
  expect_true(!is.na(inc$ir_100000_pys_high[3]))
})

test_that("check for prevalence", {
  prev <- tibble::tibble(
    incidence_analysis_id = c("1", "2", "3"),
    denominator = c(4, 6, 100),
    numerator = c(3, 4, 100),
    prev = c(0.05, 0.55, 0.55),
    prev_low = c(0.1, 0.5, 0.5),
    prev_high = c(0.15, 0.60, 0.60)
  )
